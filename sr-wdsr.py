...
class Block(chainer.Chain):
    ...
    def __init__(self, n_feats, kernel_size, res_scale=1, act=F.relu()):
        ...
        linear = 0.8
        body.append(
            F.BatchNormalization(F.Convolution2D(n_feats, n_feats*expand, 1, padding=1//2)))
        body.append(act)
        body.append(
            F.BatchNormalization(F.Convolution2D(n_feats*expand, int(n_feats*linear), 1, padding=1//2)))
        body.append(
            F.BatchNormalization(F.Convolution2D(int(n_feats*linear), n_feats, kernel_size, padding=kernel_size//2)))
...
class MODEL(chainer.Chain):
    def __init__(self, args):
        ...
        head = []
        head.append(
            F.BatchNormalization(F.Convolution2D(args.n_colors, n_feats, 3, padding=3//2)))
        ...
        tail.append(
            F.BatchNormalization(F.Convolution2D(n_feats, out_feats, 3, padding=3//2)))
        tail.append(F.depth2space(scale))

        skip = []
        skip.append(
            F.BatchNormalization(F.Convolution2D(args.n_colors, out_feats, 5, padding=5//2))
        )
